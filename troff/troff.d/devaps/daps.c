...

{
	...
	switch ( str[0] )  {					/* str[0] determines the command */
		...
		case 'T':							/* set device name */
					GET_STR(fp, rdevname);	/* MACRO - get device string */
					if ( strcmp(rdevname, "aps") != SAME_STR )
						error(FATAL, "illegal typesetter %s", rdevname);
					break;
...

{
	...
	if ( x_stat & DO_ACCT )  {				/* got accounting to do */
		...
		fprintf(fp_acct, " user = %-10s", passwd->pw_name);
...

{
	...
	sprintf(temp, "%s/dev%s/DESC.out", fontdir, rdevname);
...

{
	...
	if ( fdir == NULL  ||  fdir[0] == '\0' )	/* no alternate directory */
		 sprintf(temp, "%s/dev%s/%s.out", fontdir, rdevname, name);
...

{
	...
	sprintf(cmd, "%s/dev%s/%s.add", fontdir, rdevname, fontname[font].name);
