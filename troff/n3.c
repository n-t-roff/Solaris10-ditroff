...
filep incoff(register filep p);		/*get next blist[] block*/
filep finds(register int mn);
	...
	if (debug & DB_ALLC) {
		...
		if (oldmn >= 0 && oldmn < NM) {
			...
			if ((cc2 = (contab[oldmn].rq >> BYTE) & 0177) == 0)
				cc2 = ' ';
			fprintf(stderr, "oldmn %d %c%c, ", oldmn, cc1, cc2);
		}
		fprintf(stderr, "newmn %d; nextb was %x, will be %x\n",
			newmn, nextb, j);
	}
#endif	/* DEBUG */
	return(nextb = j);
...
int	x;
{
	...
	if (j = (unsigned long)i % sizeof(int)) {	/*check alignment for 3B*/
...
stackdump()	/* dumps stack of macros in process */
{
	...
	if (frame != stk) {
		for (p = frame; p != stk; p = p->pframe)
			fprintf(stderr, "%c%c ", p->mname&0177, (p->mname>>BYTE)&0177);
		fprintf(stderr, "\n");
	}
}
