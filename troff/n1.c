...
static printn(register long n, int b);
	...
	while (--argc > 0 && (++argv)[0][0] == '-')
		...
		case 'm':
			...
			if (getenv("TROFFMACS") != '\0') {
			  ...
			  if (tryfile(TMACDIR "/", &argv[0][2], nmfi)
			  || tryfile(TMACDIR "/tmac.", &argv[0][2], nmfi))
				nmfi++;
			  else {
	...
	char *s, *s1, *s2, *s3, *s4, *s5;
{
	...
	fprintf(stderr, s, s1, s2, s3, s4, s5);
	if (numtab[CD].val > 0)
		fprintf(stderr, gettext("; line %d, file %s"), numtab[CD].val,
			 cfname[ifi]);
	fprintf(stderr, "\n");
	stackdump();
#ifdef	DEBUG
...
#ifdef	DEBUG
	if (debug & DB_MAC)
		fprintf(stderr, "control: macro %c%c, contab[%d]\n",
