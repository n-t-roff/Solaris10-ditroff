...
char		rdevname[20] = "";	/* job is formatted for this printer */
 ...
 */
    ...
    if ( s1 == NULL || s1[0] == '\0' )
	sprintf(temp, "%s/dev%s/%s.out", fontdir, rdevname, s);
    else sprintf(temp, "%s/%s.out", s1, s);

    if ( (fin = open(temp, 0)) < 0 )  {
	sprintf(temp, "%s/dev%s/%s.out", fontdir, rdevname, mapfont(s));
	if ( (fin = open(temp, 0)) < 0 )
 ...
 */
    ...
    sprintf(temp, "%s/dev%s/fontmaps/%s", fontdir, realdev, rdevname);
