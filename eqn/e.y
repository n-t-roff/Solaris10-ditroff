...
%term <token>	MATRIX LCOL CCOL RCOL COL
%term <token>	MARK LINEUP
%term <token>	SUM INT PROD UNION INTER
%term <token>	LPILE PILE CPILE RPILE ABOVE
%term <token>	DEFINE TDEFINE NDEFINE DELIM GSIZE GFONT INCLUDE
%type <str>	text
%type <token>	eqn box lineupbox matrix column lcol ccol rcol col sbox tbox
%type <token>	size back down diacrit from lpile right left cpile pile up
%type <token>	pbox rpile to fwd font sub sup int
%right	FROM TO
%left	OVER SQRT
%right	SUP SUB
